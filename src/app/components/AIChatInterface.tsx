...
export function AIChatInterface() {
  const { messages, input, handleInputChange, setMessages, status } =
    useAssistant({
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {messages.length === 0 ? (
            <div className="text-gray-900 text-center mt-8">
              Hi! Tell me about yourself and what brings you to Bitscopic.com?
            </div>
          ) : (
        ...
        {messages.length > 0 &&
          ...
          !ratedMessages.has(messages[messages.length - 1].id) && (
            <RatingWidget
              ...
              onRatingComplete={() =>
                handleRatingComplete(messages[messages.length - 1].id)
              }
            />
          )}
      </div>
